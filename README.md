...
- milestone 14：和 miko 刻意记录和维护个人数字，增援 AI 的未来。用于更好地分析和优化自己，优化后端的数据存储和知识管理（PKM），以更加多的个人 context 。数据采集+安全的数据存储+安全调用，供前端应用使用，eg 健康和生物设备的数据、录音分析、24/7 屏幕监控和分析、录制周围环境。可能会有手机端和硬件结合。
