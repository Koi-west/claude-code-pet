 ...
 *
 * Environment variable parsing, model configuration, and PATH enhancement for desktop apps.
 * Based on Claudian's excellent environment management design.
 */
 ...
 * GUI apps like Electron have minimal PATH, so we add common locations.
 */
 ...
 * GUI apps like Electron have minimal PATH, so we need to add standard locations
...
/**
 * Parse PATH string into array of directories, handling quotes.
 */
export function parsePathEntries(pathString: string): string[] {
  ...
  for (let i = 0; i < pathString.length; i++) {
    ...
    if (char === '"' || char === "'") {
      inQuote = !inQuote;
    } else if (char === PATH_SEPARATOR && !inQuote) {
      if (current.trim()) {
...
/**
 * Find the Claude Code CLI path in common locations.
 */
export function findClaudeCLIPath(): string | null {
  // 首先检查环境变量配置
  if (process.env.CLAUDE_CLI_PATH) {
    const configuredPath = process.env.CLAUDE_CLI_PATH;
    if (isExistingFile(configuredPath)) {
      console.log(`Using Claude CLI from environment variable: ${configuredPath}`);
      return configuredPath;
    }
    console.warn(`Configured Claude CLI path not found: ${configuredPath}`);
  }
  ...
  // npm global cli.js paths
  const npmCliJsPaths = isWindows
    ? [
  ...
  // Check npm cli.js paths
  for (const p of npmCliJsPaths) {
    if (isExistingFile(p)) {
      return p;
    }
  }
  ...
  const pathDirs = parsePathEntries(envPath);
